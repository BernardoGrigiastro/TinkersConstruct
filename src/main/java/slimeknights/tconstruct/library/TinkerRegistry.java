  ...
  ---------------------------------------------------------------------------*/
    ...
    public static void addMaterial(Material material) {
        ...
        if (materials.containsKey(material.identifier)) {
            ...
            error(String.format("Could not register Material \"%s\": It was already registered by %s", material.identifier, registeredBy.getName()));
            return;
        }
    ...
    public static void addMaterialStats(String materialIdentifier, IMaterialStats stats) {
        ...
        if (!materials.containsKey(materialIdentifier)) {
            error(String.format("Could not add Stats \"%s\" to \"%s\": Unknown Material", stats.getIdentifier(), materialIdentifier));
            return;
        }
    ...
    public static void addMaterialStats(Material material, IMaterialStats stats) {
        ...
        if (material.getStats(stats.getIdentifier()) != null) {
            ...
            error(String.format("Could not add Stats to \"%s\": Stats of type \"%s\" were already registered by %s. Use the events to modify stats.", identifier, stats.getIdentifier(), registeredBy));
            return;
        }
    ...
    public static void addMaterialTrait(String materialIdentifier, ITrait trait, String stats) {
        ...
        if (!materials.containsKey(materialIdentifier)) {
            error(String.format("Could not add Trait \"%s\" to \"%s\": Unknown Material", trait.getIdentifier(), materialIdentifier));
            return;
        }
    ...
    public static boolean checkMaterialTrait(Material material, ITrait trait, String stats) {
        ...
        if (material.hasTrait(trait.getIdentifier(), stats)) {
            ...
            error(String.format("Could not add Trait to \"%s\": Trait \"%s\" was already registered by %s", identifier, trait.getIdentifier(), registeredBy));
            return false;
        }
    ...
    public static void registerStencilTableCrafting(ItemStack stencil) {
        if (!(stencil.getItem() instanceof IPattern)) {
            error(String.format("Stencil Table Crafting has to be a pattern (%s)", stencil.toString()));
            return;
        }
    ...
    public static FluidStack getMeltingForEntity(Entity entity) {
        ...
        return Optional.ofNullable(fluidStack).map(slimeknights.tconstruct.library.utils.FluidUtil::getValidFluidStackOrNull).orElse(null);
    }
