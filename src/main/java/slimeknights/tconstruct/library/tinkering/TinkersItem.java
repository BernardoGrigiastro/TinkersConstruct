...
public abstract class TinkersItem extends Item implements ITinkerable, IModifyable, IRepairable {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, TooltipContext flagIn) {
        ...
        if (!shift && !ctrl) {
            ...
            if (worldIn != null) {
                tooltip.add(TextFormat.field_1078 + I18n.translateToLocalFormatted("attribute.modifier.plus.0", Util.df.format(ToolHelper.getActualDamage(stack, MinecraftClient.getMinecraft().player)), I18n.translateToLocal("attribute.name.generic.attackDamage")));
            }
        }
        // detailed data
