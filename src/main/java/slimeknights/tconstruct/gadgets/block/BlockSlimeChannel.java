...
public class BlockSlimeChannel extends EnumBlock<SlimeType> implements BlockEntityProvider {
    ...
    public BlockSlimeChannel() {
        ...
        this.setDefaultState(this.getBlockState().getBaseState().withProperty(TYPE, SlimeType.GREEN).withProperty(SIDE, EnumFacing.DOWN).withProperty(DIRECTION, ChannelDirection.NORTH).withProperty(POWERED, Boolean.FALSE).withProperty(CONNECTED, ChannelConnected.NONE));
        //this.side = side;
        this.setHardness(0f);
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(TYPE, SlimeType.fromMeta(meta & 7)).withProperty(POWERED, (meta & 8) > 0);
    }
    ...
    private IBlockState addDataFromTE(IBlockState state, IBlockAccess source, BlockPos pos) {
        ...
        if (te instanceof TileSlimeChannel) {
            ...
            return state.withProperty(SIDE, channel.getSide()).withProperty(DIRECTION, channel.getDirection());
        }
        return state;
    }
    ...
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, EnumHand hand) {
        ...
        return this.getDefaultState().withProperty(TYPE, SlimeType.fromMeta(meta)).withProperty(SIDE, facing.getOpposite()).withProperty(DIRECTION, getPlacement(facing.getOpposite(), hitX, hitY, hitZ, placer));
    }
    ...
    public Boolean isEntityInsideMaterial(IBlockAccess world, BlockPos pos, IBlockState state, Entity entity, double yToTest, Material material, boolean testingHead) {
        ...
        else if (state.getValue(CONNECTED) == ChannelConnected.OUTER && entityAABB.intersects(getSecondaryBounds(state).offset(pos))) {
            return Boolean.TRUE;
        }
    ...
    public boolean shouldSideBeRendered(IBlockState state,
            @Nonnull IBlockAccess blockAccess, @Nonnull BlockPos pos, EnumFacing face) {
        @SuppressWarnings("unused") int knightminers_sanity_percentage_after_writing_function = 14;
        ...
        switch (connected) {
            ...
            case INNER:
                ...
                if (offsetSide == side || offsetSide == face.getOpposite() || offsetSide == flow) {
                    return offsetConnected == ChannelConnected.INNER && (offsetFlow == side.getOpposite() || offsetFlow == face || offsetFlow == flow.getOpposite());
                }
