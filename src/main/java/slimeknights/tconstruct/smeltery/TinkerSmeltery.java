...
public class TinkerSmeltery extends TinkerPulse {
    ...
    @SidedProxy(clientSide = "slimeknights.tconstruct.smeltery.SmelteryClientProxy",
    ...
    public static void registerAlloys() {
        ...
        // 1000 + 288 + 576 = 576
        // 250 + 72 + 144 = 144
        TinkerRegistry.registerAlloy(new FluidStack(TinkerFluids.clay, 144), new FluidStack(FluidRegistry.WATER, 250), new FluidStack(TinkerFluids.searedStone, 72), new FluidStack(TinkerFluids.dirt, 144));
        ...
        // 144 + 99 + 222 = 144
        TinkerRegistry.registerAlloy(new FluidStack(TinkerFluids.pigIron, 144), new FluidStack(TinkerFluids.iron, 144), new FluidStack(TinkerFluids.blood, 40), new FluidStack(TinkerFluids.clay, 72));
    ...
    public static void registerToolpartMeltingCasting(Material material) {
            ...
            if (toolPart instanceof MaterialItem) {
                ...
                if (Config.claycasts) {
                    ...
                    for (FluidStack fs : clayCreationFluids) {
                        TinkerRegistry.registerTableCasting(new CastingRecipe(clayCast, RecipeMatch.ofNBT(stack), fs, true, true));
                    }
                }
            }
        }
        ...
        if (castShard != null) {
            ...
            // register cast creation from the toolparts
            for (FluidStack fs : castCreationFluids) {
                TinkerRegistry.registerTableCasting(new CastingRecipe(castShard, RecipeMatch.ofNBT(stack), fs, true, true));
            }
        }
    }
    ...
    public static void registerOredictMeltingCasting(Fluid fluid, String ore) {
        ...
        // ingot casting
        TinkerRegistry.registerTableCasting(new PreferenceCastingRecipe(ingotOre.getLeft(), RecipeMatch.ofNBT(castIngot), fluid, ingotOre.getRight()));
        // nugget casting
        TinkerRegistry.registerTableCasting(new PreferenceCastingRecipe(nuggetOre.getLeft(), RecipeMatch.ofNBT(castNugget), fluid, nuggetOre.getRight()));
        // block casting
        TinkerRegistry.registerBasinCasting(new PreferenceCastingRecipe(blockOre.getLeft(), null, // no cast
                fluid, blockOre.getRight()));
        // plate casting
        TinkerRegistry.registerTableCasting(new PreferenceCastingRecipe(plateOre.getLeft(), RecipeMatch.ofNBT(castPlate), fluid, plateOre.getRight()));
        // gear casting
        TinkerRegistry.registerTableCasting(new PreferenceCastingRecipe(gearOre.getLeft(), RecipeMatch.ofNBT(castGear), fluid, gearOre.getRight()));
    ...
    public static void registerRecipeOredictMelting() {
        ...
        for (Recipe irecipe : CraftingManager.REGISTRY) {
            ...
            if (known.keySet().size() == 1) {
                ...
                log.trace("Added automatic melting recipe for {} ({} {})", output.toString(), amount, fluid.getName());
            }
        }
        // how fast were we?
    ...
    private void registerMeltingCasting() {
        ...
        TinkerRegistry.registerMelting(MeltingRecipe.forAmount(RecipeMatch.of("stone", Material.VALUE_SearedMaterial), TinkerFluids.searedStone, Material.VALUE_Ore()));
        TinkerRegistry.registerMelting(MeltingRecipe.forAmount(RecipeMatch.of("cobblestone", Material.VALUE_SearedMaterial), TinkerFluids.searedStone, Material.VALUE_Ore()));
        ...
        for (IToolPart toolPart : TinkerRegistry.getToolParts()) {
            if (toolPart.canBeCasted()) {
                if (toolPart instanceof MaterialItem) {
                    ...
                    TinkerRegistry.registerMelting(MeltingRecipe.forAmount(RecipeMatch.ofNBT(stack, (toolPart.getCost() * Material.VALUE_SearedMaterial) / Material.VALUE_Ingot), TinkerFluids.searedStone, (int) (toolPart.getCost() * Config.oreToIngotRatio)));
                }
            }
        }
        ...
        TinkerRegistry.registerBasinCasting(new CastingRecipe(new ItemStack(searedFurnaceController), RecipeMatch.of(Blocks.FURNACE), new FluidStack(TinkerFluids.searedStone, Material.VALUE_SearedMaterial * 8), true, true));
        
        // seared glass convenience recipe
        TinkerRegistry.registerBasinCasting(new CastingRecipe(new ItemStack(searedGlass, 1, BlockSearedGlass.GlassType.GLASS.getMeta()), RecipeMatch.of("blockGlass"), new FluidStack(TinkerFluids.searedStone, Material.VALUE_SearedMaterial * 4), true, true));
        ...
        TinkerRegistry.registerBasinCasting(new CastingRecipe(new ItemStack(Blocks.HARDENED_CLAY), RecipeMatch.of(new ItemStack(Blocks.STAINED_HARDENED_CLAY, 1, OreDictionary.WILDCARD_VALUE)), new FluidStack(FluidRegistry.WATER, 250), 150, true, false));
