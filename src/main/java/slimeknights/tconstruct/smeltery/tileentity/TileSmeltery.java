...
public class TileSmeltery extends TileHeatingStructureFuelTank<MultiblockSmeltery> implements ITickable, IInventoryGui, ISmelteryTankHandler {
    ...
    private void updateInsideCheck() {
        if (insideCheck == null || insideCheck.getX() < minPos.getX() || insideCheck.getY() < minPos.getY() || insideCheck.getZ() < minPos.getZ() || insideCheck.getX() > maxPos.getX() || insideCheck.getY() > maxPos.getY() || insideCheck.getZ() > maxPos.getZ()) {
            insideCheck = minPos;
        }
    }
            ...
            while (matched > 0) {
                ...
                for (FluidStack liquid : recipe.getFluids()) {
                    ...
                    if (!drained.isFluidEqual(toDrain) || drained.amount != toDrain.amount) {
                        log.error("Smeltery alloy creation drained incorrect amount: was {}:{}, should be {}:{}", drained.getUnlocalizedName(), drained.amount, toDrain.getUnlocalizedName(), toDrain.amount);
                    }
                }
                ...
                if (filled != recipe.getResult().amount) {
                    log.error("Smeltery alloy creation filled incorrect amount: was {}, should be {} ({})", filled, recipe.getResult().amount * matched, recipe.getResult().getUnlocalizedName());
                    break;
                }
                matched -= filled;
            }
