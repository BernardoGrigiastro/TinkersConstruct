...
public class SmelteryRenderer extends SmelteryTankRenderer<TileSmeltery> {
    ...
    public void render(@Nonnull
            TileSmeltery smeltery, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        for (int i = 0; i < smeltery.getSizeInventory(); i++) {
            if (smeltery.isStackInSlot(i)) {
                ...
                OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, brightness % 0x10000 / 1f, brightness / 0x10000 / 1f);
