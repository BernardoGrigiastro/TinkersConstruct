...
public class SlimeIslandGenerator implements IWorldGenerator {
    ...
    public void generateIsland(Random random, World world, int xPos, int zPos, int ySurfacePos, IBlockState dirt, IBlockState grass, IBlockState vine, SlimeLakeGenerator lakeGenerator, SlimeTreeGenerator treeGenerator, SlimePlantGenerator plantGen) {
                ...
                for (int y = 0; y <= erode_height; y++) {
                    ...
                    for (BlockPos pos : new BlockPos[]{pos1, pos2}) {
                        if (world.getBlockState(pos) == dirt) {
                            if (world.getBlockState(pos.add(-1, +1, 0)) != dirt || world.getBlockState(pos.add(+1, +1, 0)) != dirt || world.getBlockState(pos.add(0, +1, -1)) != dirt || world.getBlockState(pos.add(-1, +1, +1)) != dirt || random.nextInt(100) <= RANDOMNESS) {
            ...
            for (int z = 0; z <= zRange; z++) {
                for (int y = 0; y <= erode_height; y++) {
                    ...
                    for (BlockPos pos : new BlockPos[]{pos1, pos2}) {
                        ...
                        if (world.getBlockState(below.north()) != dirt || world.getBlockState(below.east()) != dirt || world.getBlockState(below.south()) != dirt || world.getBlockState(below.west()) != dirt) {
        ...
        data.getIslands().add(new StructureBoundingBox(start.getX(), start.getY(), start.getZ(), start.getX() + xRange, start.getY() + yRange, start.getZ() + zRange));
        data.markDirty();
    }
    ...
    public void tryPlacingVine(Random random, World world, BlockPos below, int limit, IBlockState vine) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if (vine.getBlock().canPlaceBlockOnSide(world, pos, EnumFacing.NORTH) || vine.getBlock().canPlaceBlockOnSide(world, pos, EnumFacing.EAST) || vine.getBlock().canPlaceBlockOnSide(world, pos, EnumFacing.SOUTH) || vine.getBlock().canPlaceBlockOnSide(world, pos, EnumFacing.WEST)) {
