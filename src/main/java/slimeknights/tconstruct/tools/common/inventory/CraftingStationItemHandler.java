...
public class CraftingStationItemHandler extends ConfigurableInvWrapperCapability {
    ...
    private void updateRecipeInOpenGUIs() {
        if (!tile.getWorld().isRemote) {
            tile.getWorld().playerEntities.stream().filter(player -> player.openContainer instanceof ContainerCraftingStation).forEach(player -> ((ContainerCraftingStation) player.openContainer).onCraftMatrixChanged());
        }
    }
}
