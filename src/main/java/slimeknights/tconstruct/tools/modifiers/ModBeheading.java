...
public class ModBeheading extends ToolModifier {
    ...
    private int getBeheadingLevel(DamageSource source) {
        ...
        ItemStack item = CapabilityTinkerProjectile.getTinkerProjectile(source).map(ITinkerProjectile::getItemStack).orElse(((EntityLivingBase) source.getTrueSource()).getHeldItem(EnumHand.MAIN_HAND));
