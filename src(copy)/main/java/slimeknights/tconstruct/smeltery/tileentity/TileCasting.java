...
public abstract class TileCasting extends TileTable implements ITickable, SidedInventory, IProgress, IFluidPacketReceiver {
        ...
        if (tank.getFluidAmount() == tank.getCapacity()) {
            ...
            if (!getWorld().isRemote) {
                if (timer >= recipe.getTime()) {
                    ...
                    if (event.consumeCast) {
                        ...
                        for (EntityPlayer player : getWorld().playerEntities) {
                            if (player.getDistanceSq(pos) < 1024 && player instanceof EntityPlayerMP) {
                                TinkerNetwork.sendPacket(player, new SPacketParticles(EnumParticleTypes.FLAME, false, pos.getX() + 0.5f, pos.getY() + 1.1f, pos.getZ() + 0.5f, 0.25f, 0.0125f, 0.25f, 0.005f, 5));
                            }
                        }
                    }
                    ...
                    getWorld().playSound(null, pos, SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.field_15256, 0.07f, 4f);
