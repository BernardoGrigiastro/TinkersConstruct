...
public class BlockTable extends BlockInventory implements BlockEntityProvider {
    ...
    /* Bounds */
    private static ImmutableList<BoundingBox> BOUNDS_Table = ImmutableList.of(new BoundingBox(0, 0.75, 0, 1, 1, 1), new BoundingBox(0, 0, 0, 0.25, 0.75, 0.25), new BoundingBox(0.75, 0, 0, 1, 0.75, 0.25), new BoundingBox(0.75, 0, 0.75, 1, 0.75, 1), new BoundingBox(0, 0, 0.75, 0.25, 0.75, 1));
    ...
    public static HitResult raytraceMultiAABB(List<BoundingBox> aabbs, BlockPos pos, Vec3d start, Vec3d end) {
        List<HitResult> list = Lists.newArrayList();
        
        for (BoundingBox axisalignedbb : aabbs) {
            list.add(rayTrace2(pos, start, end, axisalignedbb));
        }
        
        HitResult raytraceresult1 = null;
        double d1 = 0.0D;
        
        for (HitResult raytraceresult : list) {
            if (raytraceresult != null) {
                double d0 = raytraceresult.pos.squareDistanceTo(end);
    ...
    // Block.raytrace
    private static HitResult rayTrace2(BlockPos pos, Vec3d start, Vec3d end, BoundingBox boundingBox) {
        Vec3d vec3d = start.subtract((double) pos.getX(), (double) pos.getY(), (double) pos.getZ());
        Vec3d vec3d1 = end.subtract((double) pos.getX(), (double) pos.getY(), (double) pos.getZ());
        HitResult raytraceresult = boundingBox.calculateIntercept(vec3d, vec3d1);
        return raytraceresult == null ? null : new HitResult(raytraceresult.pos.addVector((double) pos.getX(), (double) pos.getY(), (double) pos.getZ()), raytraceresult.sideHit, pos);
    }
    ...
    public boolean removedByPlayer(
            ...
            @Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer player, boolean willHarvest) {
    ...
    private void writeDataOntoItemstack(
            @Nonnull ItemStack item,
            @Nonnull IBlockAccess world, @Nonnull BlockPos pos, @Nonnull IBlockState state, boolean inventorySave) {
    ...
    public void dropBlockAsItemWithChance(
            @Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull IBlockState state, float chance, int fortune) {
        if (!worldIn.isClient && !worldIn.restoringBlockSnapshots) {
