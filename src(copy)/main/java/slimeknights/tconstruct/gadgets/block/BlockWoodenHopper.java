...
public class BlockWoodenHopper extends BlockContainer {
    ...
    static {
        List<BoundingBox> commonBounds = ImmutableList.of(makeAABB(0, 10, 0, 16, 16, 16), makeAABB(4, 4, 4, 12, 10, 12));
        bounds = Stream.of(EnumFacing.values()).filter(t -> t != EnumFacing.UP).collect(Collectors.toMap(a -> a, a -> new ArrayList<>(commonBounds), (u, v) -> {
    ...
    public HitResult collisionRayTrace(IBlockState blockState,
        ...
        return bounds.get(blockState.getValue(FACING)).stream().map(bb -> rayTrace(pos, start, end, bb)).anyMatch(Objects::nonNull) ? super.collisionRayTrace(blockState, worldIn, pos, start, end) : null;
    }
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, BoundingBox entityBox, List<BoundingBox> collidingBoxes,
