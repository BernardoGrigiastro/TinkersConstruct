        ...
        if (worldIn.isAirBlock(below)) {
            ...
            if (freeFloating(worldIn, pos, state)) {
                ...
                if (i > 2 || rand.nextInt(2) == 0) {
                    state = nextStage.getDefaultState().withProperty(NORTH, state.getValue(NORTH)).withProperty(EAST, state.getValue(EAST)).withProperty(SOUTH, state.getValue(SOUTH)).withProperty(WEST, state.getValue(WEST));
                }
            }
