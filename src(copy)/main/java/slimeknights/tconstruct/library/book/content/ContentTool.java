...
public class ContentTool extends TinkerPage {
    ...
    @SerializedName("tool") public String toolName;
    ...
    public void load() {
        ...
        if (tool == null) {
            tool = TinkerRegistry.getTools().stream().filter(toolCore -> toolName.equals(toolCore.getIdentifier())).findFirst().orElseThrow(() -> new RuntimeException("Unknown tool " + toolName));
        }
        if (parts == null) {
            parts = tool.getToolBuildComponents().stream().map(PartMaterialType::getPossibleParts).collect(Collectors.toList());
        }
    }
