...
public class CustomFontColor {
    ...
    public static String encodeColor(int r, int g, int b) {
        return String.format("%c%c%c", ((char) (MARKER + (r & 0xFF))), ((char) (MARKER + (g & 0xFF))), ((char) (MARKER + (b & 0xFF))));
    }
    ...
    public static String formatPartialAmount(int value, int max) {
        return String.format("%s%s%s/%s%s", CustomFontColor.valueToColorCode((float) value / (float) max), Util.df.format(value), TextFormat.field_1080.toString(), CustomFontColor.valueToColorCode(1f), Util.df.format(max)) + TextFormat.RESET;
    }
}
