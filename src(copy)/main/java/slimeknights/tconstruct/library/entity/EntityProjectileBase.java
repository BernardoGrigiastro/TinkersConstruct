        ...
        if (item.getItem() instanceof ToolCore && this.shootingEntity instanceof EntityLivingBase) {
            ...
            if (!getEntityWorld().isClient) {
                ...
                if (item.getItem() instanceof IProjectile) {
                    ...
                    // factor in power
                    projectileAttributes.put(EntityAttributes.ATTACK_DAMAGE.getName(), new EntityAttributeModifier(PROJECTILE_POWER_MODIFIER, "Weapon damage multiplier", tinkerProjectile.getPower() - 1f, 2));
