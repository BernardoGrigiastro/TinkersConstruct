...
public class TooltipBuilder {
    ...
    public static String formatAmmo(int durability, int ref) {
        return String.format("%s: %s%s%s/%s%s", Util.translate(LOC_Ammo), CustomFontColor.valueToColorCode((float) durability / (float) ref), df.format(durability), TextFormat.field_1080.toString(), COLOR_Durability, df.format(ref)) + TextFormat.RESET;
    }
