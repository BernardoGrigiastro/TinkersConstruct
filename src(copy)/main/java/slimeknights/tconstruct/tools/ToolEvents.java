...
public class ToolEvents {
    ...
    public void onLootTableLoad(LootTableLoadEvent event) {
        ...
        if (event.getName().equals(LootTableList.ENTITIES_WITHER_SKELETON)) {
            ...
            LootEntry entry = new LootEntryItem(TinkerCommons.matNecroticBone.getItem(), 1, 0, new LootFunction[]{new SetMetadata(lootConditions, new UniformLootTableRange(TinkerCommons.matNecroticBone.getMetadata()))}, lootConditions, "necrotic_bone");
            event.getTable().addPool(new LootPool(new LootEntry[]{entry}, new LootCondition[]{new KilledByPlayerLootCondition(false), new RandomChanceWithLootingLootCondition(0.07f, 0.05f)}, new UniformLootTableRange(1), new UniformLootTableRange(0), "necrotic_bone"));
        }
    }
    ...
    public void onLooting(LootingLevelEvent event) {
        ...
        ItemStack item = CapabilityTinkerProjectile.getTinkerProjectile(event.getDamageSource()).map(ITinkerProjectile::getItemStack).orElse(ItemStack.EMPTY);
