...
public class RangedRenderEvents {
    ...
    private static boolean isCarryingLoadedCrossbow(EntityPlayer entityPlayer, final EnumHand hand) {
        return Optional.ofNullable(entityPlayer).map(player -> player.getHeldItem(hand)).filter(stack -> stack.getItem() == TinkerRangedWeapons.crossBow).map(stack -> TinkerRangedWeapons.crossBow.isLoaded(stack)).orElse(false);
    }
}
