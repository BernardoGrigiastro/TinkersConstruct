...
public class BoltCoreCastingRecipe implements ICastingRecipe {
    ...
    public boolean matches(ItemStack cast, Fluid fluid) {
        ...
        return cast.getItem() == TinkerTools.arrowShaft && TinkerTools.arrowShaft.getMaterial(cast).hasStats(MaterialTypes.SHAFT) && isFluidWithHeadMaterial(fluid);
    }

    private boolean isFluidWithHeadMaterial(Fluid fluid) {
        return TinkerRegistry.getAllMaterials().stream().filter(mat -> mat.hasStats(MaterialTypes.HEAD)).map(Material::getFluid).anyMatch(fluid::equals);
    }
