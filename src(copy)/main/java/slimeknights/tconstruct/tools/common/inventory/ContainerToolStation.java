...
public class ContainerToolStation extends ContainerTinkerStation<TileToolStation> {
    ...
    public void onResultTaken(EntityPlayer playerIn, ItemStack stack) {
        ...
        try {
            resultTaken = !repairTool(true).isEmpty() || !replaceToolParts(true).isEmpty() || !modifyTool(true).isEmpty() || !renameTool().isEmpty();
        } catch (TinkerGuiException e) {
    ...
    private ItemStack renameTool() throws TinkerGuiException {
        ...
        // modifying possible?
        if (tool.isEmpty() || !(tool.getItem() instanceof TinkersItem) || ChatUtil.isNullOrEmpty(toolName) || tool.getDisplayName().equals(toolName)) {
            return ItemStack.EMPTY;
        }
