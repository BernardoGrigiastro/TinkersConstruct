...
public class GuiInfoPanel extends GuiModule {
    ...
    private void shiftSlider(int xd, int yd) {
        slider = new GuiWidgetSlider(sliderNormal.shift(xd, yd), sliderHover.shift(xd, yd), sliderHover.shift(xd, yd), sliderTop.shift(xd, yd), sliderBot.shift(xd, yd), sliderBar.shift(xd, yd));
    }
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (hasTooltips() && mouseX >= guiRight() - border.w - fontRenderer.getCharWidth('?') / 2 && mouseX < guiRight() && mouseY > guiTop + 5 && mouseY < guiTop + 5 + fontRenderer.fontHeight) {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (hasCaption()) {
            ...
            fontRenderer.drawStringWithShadow(TextFormat.field_1073 + TextFormat.getTextWithoutFormattingCodes(caption), guiLeft + x2, y, color);
            y += fontRenderer.fontHeight + 3;
        }
